...
static void timer_callback(struct timer_list *t)
{
    ...
    list_for_each_entry_safe(entry, tmp, &monitored_list, list) {
        ...
        if (rss < 0) {
            printk(KERN_INFO
                   "[container_monitor] Process exited container=%s"
                   " pid=%d, removing.\n",
                           ...
                           unsigned long arg)
{
    ...
    if (cmd == MONITOR_REGISTER) {
        ...
        ne->pid               = req.pid;
        ne->soft_limit_bytes  = req.soft_limit_bytes;
        ne->hard_limit_bytes  = req.hard_limit_bytes;
        ne->soft_warned       = 0;
        strncpy(ne->container_id, req.container_id,
                MONITOR_NAME_LEN - 1);
        ne->container_id[MONITOR_NAME_LEN - 1] = '\0';
        INIT_LIST_HEAD(&ne->list);

        mutex_lock(&monitored_lock);
        list_add_tail(&ne->list, &monitored_list);
        mutex_unlock(&monitored_lock);

        printk(KERN_INFO
               "[container_monitor] Registered container=%s pid=%d"
               " soft=%lu hard=%lu\n",
               ne->container_id, ne->pid,
               ne->soft_limit_bytes, ne->hard_limit_bytes);
        return 0;
    }
